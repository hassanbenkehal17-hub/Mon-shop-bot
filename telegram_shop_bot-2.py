...
def cat_kb():
    ...
    for pid, p in PRODUCTS.items():
        rows.append([InlineKeyboardButton(f"{p['name']} - {p['price']:.2f}EUR", callback_data=f"prod_{pid}")])
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "menu":
        ...
        pid = d[4:]
        if pid in cart:
            cart[pid] = cart[pid] - 1 if cart[pid] > 1 else cart.pop(pid) or 0
        qty = cart.get(pid, 0)
        p = PRODUCTS[pid]
        txt = f"{p['name']}\nPrix : {p['price']:.2f}EUR\n\n{p['desc']}"
        if qty:
        ...
        try:
            await context.bot.send_message(chat_id=ADMIN_ID, text=f"Commande de {user.full_name}:\n\n{summary}")
        except Exception as e:
            logger.warning(f"Admin error: {e}")
        context.user_data["cart"] = {}
    elif d == "contact":
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"Message de {user.full_name}:\n\n{update.message.text}")
    except Exception as e:
